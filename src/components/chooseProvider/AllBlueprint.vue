...
<template>
    ...
    <div class="blueprint-items grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-8">
          <div v-for="(item , index) in bluePrintList" :key="index">
            <BlueprintItems :item="item" :index="index + 2" v-model:defaultSelectedBlueprint="defaultSelectedBlueprint" />
          </div>
        </div>  
</template>
